...
export default apiInitializer(async (api) => {
  ...
  if (settings.apple) {
    deprecatedProviders.push("apple");
  }
  if (settings.discord) {
    deprecatedProviders.push("discord");
  }
  if (settings.facebook) {
    deprecatedProviders.push("facebook");
  }
  if (settings.github) {
    deprecatedProviders.push("github");
  }
  if (settings.google) {
    deprecatedProviders.push("google_oauth2");
  }
  if (settings.linkedin) {
    deprecatedProviders.push("linkedin_oidc");
  }
  if (settings.twitter) {
    deprecatedProviders.push("twitter");
  }
  ...
  const hasDiscourseId = currentUser.associated_accounts.some(
    ({ name }) => name === "discourse_id"
  );

  if (deprecated.length > 0 && !hasDiscourseId) {
    const providers = deprecated
      .map(({ name }) => PROVIDER_NAMES[name] || name)
      .join(", ");
    ...
    if (settings.deprecation_date) {
      ...
      message = i18n(
        themePrefix("social_logins_deprecation.notice_with_date"),
        { providers, date }
      );
    } else {
      message = i18n(themePrefix("social_logins_deprecation.notice"), {
        providers,
      });
    }

    // TODO: does it make sense to show the notice when any settings changes?
    addGlobalNotice(message, "social-logins-deprecations", {
      dismissable: true,
    });
  }
});
